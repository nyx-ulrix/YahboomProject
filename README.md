...
- **START** sends `auto_on` (same as the EXPLORE button). Stop-mode slider is disabled during an active session.
- **Stop modes (slider):** **Cache** — Pi `cache_aware_offloading.py` only; **Hybrid** — Pi script + dashboard VIT (first trigger wins); **Edge** (default) — dashboard VIT bottle stop only (`auto_off` + `stop`).
- **Cache/Hybrid:** START unlocks when the Pi script is running and bottle embedding is ready (log line or MQTT `yahboom/cache_aware/ready`). Edge mode stops the Pi script.
- Timestamps in **CSV export** use the **Raspberry Pi clock**, not the browser.
- Records **stop mode** and **stopped by** (Pi script, dashboard VIT, or manual) per run.

**APIs:** `GET /api/test_bench/stop_mode` (`?force=1` for live probe), `POST /api/test_bench/stop_mode`, `GET /api/drive_status`, `GET /api/grid_status`, `GET /api/vit/status`.

**Code:** `StopTestBenchWidget` in [Widgets.tsx](Yahboom%20Dashboard/src/app/components/Widgets.tsx); [cache_aware_ssh.py](Yahboom%20Dashboard/backend/app/services/test_bench/cache_aware_ssh.py); [test_bench_routes.py](Yahboom%20Dashboard/backend/app/routes/test_bench_routes.py); [edgeAwareStopLabelEstop.ts](Yahboom%20Dashboard/src/lib/edgeAwareStopLabelEstop.ts).
...
| yahboom/cache_aware/ready | Retained JSON: `{ready: true, label, dims}` when Pi cache-aware bottle embedding is ready                                                                                                                                                                                                                                              | [Yahboom Car/cache_aware_offloading.py](Yahboom%20Car/cache_aware_offloading.py)                            | Dashboard backend (`mqtt_service.py`) — unlocks test-bench START in cache/hybrid mode                                                                                                                                              |
| yahboom/detect/status | Bottle detection events from cache-aware Pi script (similarity, threshold, dims)                                                                                                                                                                                                                                                        | [Yahboom Car/cache_aware_offloading.py](Yahboom%20Car/cache_aware_offloading.py)                            | Logging / monitoring                                                                                                                                                                                                               |
