# webrtc_video_only.py
# Duplicate of webrtc_server.py that ONLY streams camera video over WebRTC.
# No MQTT frame publish, no /frame.jpg snapshot — use webrtc_server.py when
# VIT / YOLO need camera frames.
...
# =========================
# CAMERA WORKER (video only — no MQTT / JPEG split)
# =========================
def camera_worker():
    global latest_frame
...
<html>
<head>
  <title>Yahboom WebRTC Video (stream only)</title>
  <style>
    body {
...
<body>
  <h1>Yahboom WebRTC Live Video (stream only)</h1>
...
def main():
    ...
    print(f"\n[VIDEO ONLY] {dashboard_url}")
    print("[INFO] WebRTC stream only — no MQTT frames, no /frame.jpg")
    print("[INFO] For VIT / YOLO frame feed, run webrtc_server.py instead.\n")
