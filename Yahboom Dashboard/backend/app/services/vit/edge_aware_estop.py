...
_MIN_CONFIDENCE = float(os.getenv("EDGE_AWARE_MIN_CONFIDENCE", "75.0"))
_COOLDOWN_SEC = float(os.getenv("EDGE_AWARE_ESTOP_COOLDOWN_SEC", "5.0"))


class EdgeAwareEstopService:
    """Tracks the cache-aware / edge stop mode for the test bench."""
    ...
    def edge_aware_enabled(self) -> bool:
        """Dashboard VIT bottle polling is always on — edge stop is always armed."""
        return True
